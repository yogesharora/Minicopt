...
void peepHoleOptimization(inst_t instList);
...
//	codegen_entry(fptr);
	...
	/* 1. perform peephole optimization */
	peepHoleOptimization(instList);

	/* 2. build liveness information*/
	livenessAnalysis analysis( instList);
	...
	while (changing)
	{
		...
		if(verbose == 1)
		{
			PRINTF("Liveliness Analysis Round %d - Info after convergence\n", ctr++);
			analysis.print();
			PRINTF("\n");
		}
		FILE *fptr = fopen(outfile, "w");
		codegen_entry(fptr);
		changing = analysis.dce(&result);
		print_list(fptr, result);
		codegen_exit(fptr);
		fclose(fptr);
	}
...
//	print_list(fptr, result); /* dump code to output file */
//	codegen_exit(fptr);
//	fclose(fptr);
}
...
void peepHoleOptimization(inst_t instList)
{
	...
	while (head)
	{
		inst_t next = head->next;
		if (next && (next->op == OP_LDR || next->op == OP_STR))
		{
			if (head->op == OP_ADD || head->op == OP_SUB)
			{
				if (head->ops[1].reg == R5 && head->ops[2].t == op_imm)
				{
					if (next->ops[1].reg == head->ops[0].reg && next->ops[2].t
							== op_imm)
					{
						int c = head->ops[2].imm;
						if(head->op==OP_SUB)
							c=-c;
						next->ops[1].reg = R5;
						next->ops[2].imm+=c;
