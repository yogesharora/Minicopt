...
livenessAnalysis::livenessAnalysis(inst_t inst) :
	instructions(inst), liveInfo(NULL)
{
	initProgramInfo();
...
bool livenessAnalysis::dce(inst_t *result)
{
	int i = 0;
	inst_t cur = instructions;
	inst_t prev = NULL;
	bool changed = false;
	...
	deleteOldLiveInfo();
	noInstruction -= totalDeletions;
	*result = instructions;
	return changed;
}

bool livenessAnalysis::deleteFromList(inst_t cur, inst_t prev)
{
	bool deleted = false;
	if (cur->label != NULL)
	{
		if (cur->next != NULL)
		{
			...
			{
				// move the label to next
				deleted = true;
				cur->next->label = cur->label;
				prev->next = cur->next;
			}
		}
		else
	...
	else
	{
		...
		if (prev != NULL)
			...
			instructions = cur->next;
	}

	return deleted;
}

void livenessAnalysis::makeNop(inst_t cur)
{
	cur->op = OP_NOP;
}
